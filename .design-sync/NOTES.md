...
- **This resync uploaded into the pre-existing "Application UI Kit (synced)" project** (`projectId` now pinned in `config.json`) via the atomic path: full writes + 105 explicit deletes of every old `Application*`-prefixed path (verified via `list_files` before AND after — confirmed 0 stale `Application*` paths remain post-upload). `_adherence.oxlintrc.json` and `_ds_manifest.json` are app-managed and were left untouched (not in the plan's writes or deletes).
