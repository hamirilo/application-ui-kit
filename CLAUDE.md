...
アプリケーション側の依存名は `application-ui-kit` に固定する。
publish される実パッケージは `@<owner>/application-ui-kit` だが、利用側 `package.json` で npm alias を設定し、
実装コード・Story・JSDoc では常に `application-ui-kit` を import する。詳細は README「Package」。
