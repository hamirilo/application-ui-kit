...
export interface HamiriloFormFieldProps {
  /** ラベル文字列 */
  ...
  /**
   * 入力欄。`<HamiriloInput>` / `<HamiriloSelect>` / `<textarea>` 等を渡す。
...
/**
 * HamiriloFormField コンポーネント
 *
 * @example
 * ```tsx
 * <HamiriloFormField label="件名" required>
 *   <HamiriloInput placeholder="例: 〇〇の改善について" />
 * </HamiriloFormField>
 *
 * // エラー付き（error を渡すと子の HamiriloInput も自動で赤枠になる）
 * <HamiriloFormField label="メールアドレス" error="形式が正しくありません">
 *   <HamiriloInput defaultValue="foo@" />
 * </HamiriloFormField>
 *
 * // ヘルプ付き
 * <HamiriloFormField label="公開範囲" helpText="後から変更できます">
 *   <HamiriloSelect items={items} />
 * </HamiriloFormField>
 * ```
 */
export function HamiriloFormField({
  label,
  required = false,
  error,
  helpText,
  ...
  // error は子が受け取れる場合（HamiriloInput / HamiriloSelect）だけ渡す。
...
HamiriloFormField.displayName = "HamiriloFormField";
