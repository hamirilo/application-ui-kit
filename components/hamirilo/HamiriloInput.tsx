...
export interface HamiriloInputProps extends Omit<React.ComponentPropsWithoutRef<"input">, "size"> {
  /**
   * エラー状態。true にすると枠線が danger 色になる。
   *
   * <important>
   * 色だけでエラーを表現してはいけない（色覚特性のある利用者に伝わらない）。
   * 必ずエラーメッセージも表示する。HamiriloFormField を使えば自動で両方付く。
   * </important>
   */
  error?: boolean;
...
/**
 * HamiriloInput コンポーネント
 *
 * @example
 * ```tsx
 * // 基本
 * <HamiriloInput placeholder="タイトルを入力" />
 *
 * // エラー状態（メッセージは別途表示すること）
 * <HamiriloInput error defaultValue="不正な値" />
 *
 * // アイコン付き
 * <HamiriloInput leftIcon={<Search className="w-4 h-4" />} placeholder="検索" />
 *
 * // ラベル・エラー・ヘルプをまとめる場合は HamiriloFormField を使う
 * <HamiriloFormField label="件名" required error="件名は必須です">
 *   <HamiriloInput error />
 * </HamiriloFormField>
 * ```
 */
export const HamiriloInput = React.forwardRef<HTMLInputElement, HamiriloInputProps>(
...
HamiriloInput.displayName = "HamiriloInput";
