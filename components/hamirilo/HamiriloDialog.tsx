/**
 * HamiriloDialog - shared UI libraryの汎用ダイアログコンポーネント
 *
 * shadcn/ui の Dialog をラップし、プロジェクト標準のダイアログ UI を提供します。
 * 画面側では HamiriloDialog のみを使用し、shadcn/ui の Dialog を直接使用しないでください。
 */
...
export interface HamiriloDialogProps {
  /**
   * ダイアログの開閉状態
   */
  open: boolean;
 ...
 *       <HamiriloButton variant="secondary">アクション1</HamiriloButton>
 *       <HamiriloButton variant="primary">アクション2</HamiriloButton>
 *     </div>
 *   }
 * >
 *   <div>コンテンツ</div>
...
export const HamiriloDialog = ({
  ...
  const defaultFooter = (
    <>
      ...
      {confirmText && (
        <HamiriloButton variant={confirmVariant} onClick={onConfirm} loading={confirmLoading}>
          {confirmText}
        </HamiriloButton>
      )}
    </>
