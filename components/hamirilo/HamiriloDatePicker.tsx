...
export type HamiriloDatePickerMode = "single" | "range" | "multiple";
export type HamiriloDatePickerValue = Date | DateRange | Date[];

export interface HamiriloDatePickerProps {
  /**
   * 選択モード
   * - single: 単一日付選択
   ...
   */
  value?: HamiriloDatePickerValue;

  /**
   * 値変更時のコールバック
   */
  onChange?: (value: HamiriloDatePickerValue | undefined) => void;
...
function formatValue(mode: HamiriloDatePickerMode, value?: HamiriloDatePickerValue): string | null {
...
/**
 * HamiriloDatePicker コンポーネント
 *
 * @example
 * ```tsx
 * // 単一日付選択
 * const [date, setDate] = useState<Date>()
 * <HamiriloDatePicker mode="single" value={date} onChange={(v) => setDate(v as Date)} />
 *
 * // 日付範囲選択
 * const [range, setRange] = useState<DateRange>()
 * <HamiriloDatePicker mode="range" value={range} onChange={(v) => setRange(v as DateRange)} />
 *
 * // 複数日選択（日程調整の候補日など）
 * const [dates, setDates] = useState<Date[]>([])
 * <HamiriloDatePicker mode="multiple" value={dates} onChange={(v) => setDates((v as Date[]) ?? [])} />
 * ```
 */
export function HamiriloDatePicker({
  mode = "single",
  value,
  onChange,
  placeholder = "日付を選択",
...
HamiriloDatePicker.displayName = "HamiriloDatePicker";
