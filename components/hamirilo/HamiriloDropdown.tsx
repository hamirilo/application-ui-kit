/**
 * HamiriloDropdown - shared UI libraryのドロップダウンメニューコンポーネント
 *
 * 行アクション（編集・削除・複製など）や、テーブル・カード上のメニューボタンで使用します。
 * shadcn/ui の DropdownMenu をラップし、`items` 配列を渡すだけで構築できる簡潔な API を提供します。
 *
 * 画面側では HamiriloDropdown のみを使用し、shadcn/ui の DropdownMenu を直接使用しないでください。
 */
...
export interface HamiriloDropdownItem {
  /**
   * メニュー項目の一意なキー
   */
  key: string;
...
export interface HamiriloDropdownProps {
  /**
   * メニューを開くトリガー要素（通常は HamiriloButton や IconButton）
   */
 ...
 * <HamiriloDropdown
 *   trigger={
 *     <HamiriloButton variant="ghost" size="icon">
 ...
 * ```
 */
export const HamiriloDropdown = ({
  trigger,
  items,
  label,
  align = "end",
}: HamiriloDropdownProps) => {
  return (
    <DropdownMenu>
