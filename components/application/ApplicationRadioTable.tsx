/**
 * ApplicationRadioTable - 表から1行を選ばせる（排他選択つきの一覧）
 *
 * 「候補を複数の属性で見比べてから1つ選ぶ」ための部品。プラン選択・
 * 送付先選択・取り込み対象の選択など、ラベルと説明だけでは決められず
 * 列で並べて比較したい場面に使う。
 *
 * 見た目と空状態は `ApplicationTable` に委ね、先頭にラジオ列を足しているだけ。
 * 一覧の描画規約（列定義・空状態必須）は ApplicationTable と同じものが効く。
 *
 * <important>
...
import { ApplicationTable, type ApplicationTableColumn } from "./ApplicationTable";

export interface ApplicationRadioTableProps<T> {
  /** 列定義（ラジオ列は自動で先頭に足される） */
  columns: ApplicationTableColumn<T>[];
 ...
 * ```
 */
export function ApplicationRadioTable<T>({
  columns,
  rows,
  rowValue,
  rowLabel,
  ...
  const radioColumn: ApplicationTableColumn<T> = {
    key: "__radio",
...
ApplicationRadioTable.displayName = "ApplicationRadioTable";
