...
describe("splitCreatedValues", () => {
  it("既存の値と新規作成された名前を分ける", () => {
    expect(splitCreatedValues(["12", `${APPLICATION_COMBOBOX_CREATE_PREFIX}新しいタグ`])).toEqual({
      values: ["12"],
      created: ["新しいタグ"],
    });
  });
...
describe("ApplicationCombobox", () => {
  it("入力した文字列でラベルを絞り込む", async () => {
    render(<ApplicationCombobox items={items} aria-label="店舗" />);
    await openWith("新宿");
  ...
  it("該当が無いときは emptyMessage を出す", async () => {
    render(<ApplicationCombobox items={items} aria-label="店舗" emptyMessage="ありません" />);
    await openWith("横浜");
  ...
  it("creatable のとき、一覧に無い文字列で新規作成の項目を出す", async () => {
    render(<ApplicationCombobox items={items} creatable aria-label="店舗" />);
    await openWith("横浜店");
  ...
  it("新規作成を選ぶと接頭辞付きの値を返し、onCreate も呼ぶ", async () => {
    ...
    await waitFor(() =>
      expect(onValueChange).toHaveBeenCalledWith(`${APPLICATION_COMBOBOX_CREATE_PREFIX}横浜店`),
    );
    expect(onCreate).toHaveBeenCalledWith("横浜店");
  });

  it("multiple のとき、選択済みをチップで出す", () => {
    render(<ApplicationCombobox items={items} multiple value={["1", "3"]} aria-label="店舗" />);
  ...
  it("multiple のとき、新規作成された値も名前でチップに出す", () => {
    render(
      <ApplicationCombobox
        ...
        multiple
        creatable
        value={["1", `${APPLICATION_COMBOBOX_CREATE_PREFIX}横浜店`]}
        aria-label="店舗"
      />,
    );
  ...
  it("error のとき入力欄を aria-invalid にする", () => {
    render(<ApplicationCombobox items={items} error aria-label="店舗" />);
