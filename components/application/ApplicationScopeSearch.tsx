/**
 * ApplicationScopeSearch - 1本の入力で複数種別を横断検索する（種別グループ付きサジェスト）
 *
 ...
 * 選択結果をフォームに入れたいなら ApplicationCombobox（1種類の候補から選ぶ）か
 * ApplicationTreeSelect（階層から選ぶ）を使う。
 *
...
export interface ApplicationScopeSearchProps {
  /** 検索対象の候補 */
  items: ApplicationScopeSearchItem[];
  ...
  /** 候補を決定したときに呼ばれる */
  onSelect?: (item: ApplicationScopeSearchItem) => void;
...
function kindsFromItems(items: ApplicationScopeSearchItem[]): string[] {
  const seen: string[] = [];
...
function matchesQuery(item: ApplicationScopeSearchItem, needle: string): boolean {
 ...
 * ```
 */
export const ApplicationScopeSearch = React.forwardRef<
  HTMLInputElement,
...
>(
  (
    ...
    const select = (item: ApplicationScopeSearchItem) => {
      setOpen(false);
...
ApplicationScopeSearch.displayName = "ApplicationScopeSearch";
