 ...
 * FormFieldSet は名前を aria-labelledby で結ぶ。
 */
describe("FormFieldSet", () => {
  ...
  describe.each(GROUPS)("$name", ({ selector, render: renderGroup }) => {
    ...
    it("グループの名前が legend に紐づく", () => {
      const { container } = render(<FormFieldSet label="優先度">{renderGroup()}</FormFieldSet>);
      expect(resolve(group(container), "aria-labelledby").join("")).toContain("優先度");
    });

    it("必須は文字でも伝える", () => {
      const { container } = render(
        <FormFieldSet label="優先度" required>
          {renderGroup()}
        </FormFieldSet>,
      );
    ...
    it("error でグループが aria-invalid になり、FormFieldSet が error 表示へ切り替わる", () => {
      const { container } = render(
        <FormFieldSet label="優先度" error="優先度を選択してください">
          {renderGroup()}
        </FormFieldSet>,
      );
    ...
    it("error と helpText がグループの aria-describedby に入る", () => {
      const { container } = render(
        <FormFieldSet label="優先度" error="選択してください" helpText="後から変更できます">
          {renderGroup()}
        </FormFieldSet>,
      );
    ...
    it("独自 prop を DOM へ漏らさない", () => {
      const { container } = render(
        <FormFieldSet label="優先度" error="選択してください">
          {renderGroup()}
        </FormFieldSet>,
      );
  ...
  it("FormFieldSet の error と子の aria-describedby は両方残る", () => {
    const { container } = render(
      <FormFieldSet label="優先度" error="選択してください">
  ...
  it("同じ name のグループを 2 つ載せても id が衝突しない", () => {
    const { container } = render(
      <>
        <FormFieldSet label="優先度 A">
          <RadioGroup items={ITEMS} name="priority" />
        </FormFieldSet>
        <FormFieldSet label="優先度 B">
          <RadioGroup items={ITEMS} name="priority" />
        </FormFieldSet>
      </>,
    );
  ...
  it("ラジオグループに読み上げ名が付く", () => {
    render(<FormFieldSet label="優先度">{GROUPS[0].render()}</FormFieldSet>);
   ...
   * FormFieldSet を用意した理由なので、その差を固定しておく。 */
  it("FormField ではグループに名前が付かない（FormFieldSet を使う理由）", () => {
    render(
      <FormField label="優先度">
        <RadioGroup items={ITEMS} name="priority" />
      </FormField>,
