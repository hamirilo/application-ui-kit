/**
 * ApplicationCopyButton - 共有 UI ライブラリのコピー（クリップボード）ボタン
 *
...
import { ApplicationButton, type ApplicationButtonProps } from "./ApplicationButton";
 ...
 */
export type ApplicationCopyResult = "copied" | "selected" | "failed";
...
): Promise<ApplicationCopyResult> {
  // execCommand('copy') は「現在の選択範囲」を写すため、退避先があれば先に選択しておく。
  // clipboard API が失敗したときも選択が残り、そのまま手動コピーできる。
  if (fallbackSelect) selectAll(fallbackSelect);
...
export interface ApplicationCopyButtonProps
  extends Omit<ApplicationButtonProps, "children" | "loading" | "leftIcon" | "rightIcon"> {
  /** コピーする文字列 */
  value: string;
   ...
   */
  onCopyResult?: (result: ApplicationCopyResult) => void;
}

/**
 * ApplicationCopyButton コンポーネント
 *
 * @example
 * ```tsx
 * // 基本
 * <ApplicationCopyButton value={apiToken} />
 *
 * // 値が見える入力欄と組にする（一度しか表示されない値はこの形にする）
 * const inputRef = React.useRef<HTMLInputElement>(null);
 * const [status, setStatus] = React.useState("");
 * <ApplicationInput ref={inputRef} readOnly value={issuedUrl} />
 ...
 * ```
 */
export const ApplicationCopyButton = React.forwardRef<
  HTMLButtonElement,
...
>(
  (
    ...
    const handleClick: NonNullable<ApplicationButtonProps["onClick"]> = async (event) => {
      onClick?.(event);
...
ApplicationCopyButton.displayName = "ApplicationCopyButton";
