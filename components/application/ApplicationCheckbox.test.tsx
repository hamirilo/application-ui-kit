...
describe("ApplicationCheckbox", () => {
  it("ラベルとチェックボックスを紐づける", () => {
    render(<ApplicationCheckbox label="利用規約に同意する" />);
    expect(screen.getByRole("checkbox", { name: "利用規約に同意する" })).toBeTruthy();
  });

  it("チェック済みはチェックのアイコンを出す", () => {
    render(<ApplicationCheckbox label="チェック済み" defaultChecked />);
  ...
  it("indeterminate はダッシュのアイコンを出し、data-indeterminate を持つ", () => {
    render(<ApplicationCheckbox label="すべて選択" indeterminate />);
