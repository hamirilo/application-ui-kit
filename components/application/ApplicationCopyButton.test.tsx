...
import { ApplicationCopyButton, copyTextToClipboard } from "./ApplicationCopyButton";
...
describe("ApplicationCopyButton", () => {
  it("コピー成功でラベルが copiedLabel に変わり、feedbackDuration 後に戻る", async () => {
    stubSecureContext(true);
    stubClipboard(() => Promise.resolve());

    const { getByRole } = render(<ApplicationCopyButton value="token" feedbackDuration={10} />);
  ...
  it("全滅時は failedLabel を表示し、onCopyResult へ failed を渡す", async () => {
    ...
    const { getByRole } = render(
      <ApplicationCopyButton value="token" onCopyResult={onCopyResult} />,
    );
