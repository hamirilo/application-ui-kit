...
export interface ApplicationSelectItem {
  /** 選択時の値 */
  value: string;
  /** 表示ラベル */
  label: string;
 ...
 *
 * // 基本
 * <ApplicationSelect items={items} placeholder="優先度を選択" />
 *
 * // 制御付き
 * <ApplicationSelect items={items} value={v} onValueChange={setV} />
 *
 * // グループ分け
 * <ApplicationSelect items={[
 *   { value: "a", label: "営業部", group: "本社" },
 *   { value: "b", label: "製造部", group: "工場" },
 * ]} />
 * ```
 */
export const ApplicationSelect = React.forwardRef<HTMLButtonElement, ApplicationSelectProps>(
  (
    {
      items,
      value,
    ...
    const groups = React.useMemo(() => {
      const out: Array<{ name: string | undefined; items: ApplicationSelectItem[] }> = [];
