/**
 * ApplicationTreeSelect - 階層構造から 1 つ選ぶカスケードセレクト
 *
 ...
 * 選択肢が十数個までなら ApplicationSelect、
 * 件数が多く「検索して 1 つ選ぶ」なら ApplicationCombobox を使う。
 * ApplicationTreeSelect は、選んだ結果より **どこに属するか** を見せたい場合のためにある。
 * </important>
 */
...
export interface ApplicationTreeSelectItem {
  /** 選択時の値。ツリー全体で一意にすること */
  value: string;
  /** 表示ラベル */
  label: string;
...
export interface ApplicationTreeSelectProps {
  /** 階層ツリー。ルート階層を配列で渡す */
  items: ApplicationTreeSelectItem[];
   ...
   * `ApplicationFormField` で囲まない場合は `aria-label` か `aria-labelledby` が必須。
   * </important>
   */
  "aria-label"?: string;
...
export function findTreePath(
  items: ApplicationTreeSelectItem[],
  value: string,
): ApplicationTreeSelectItem[] | null {
 ...
 * <ApplicationTreeSelect items={UNITS} value={unit} onValueChange={setUnit} aria-label="組織" />
 * ```
 */
export function ApplicationTreeSelect({
  items,
  value,
  defaultValue = null,
  onValueChange,
  ...
  const columns = React.useMemo(() => {
    const result: ApplicationTreeSelectItem[][] = [];
...
ApplicationTreeSelect.displayName = "ApplicationTreeSelect";
