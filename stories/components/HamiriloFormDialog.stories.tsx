...
/**
 * HamiriloFormDialog はフォーム入力を伴うダイアログ。
 *
 * <important>
 ...
 */
const meta = {
  title: "Components/HamiriloFormDialog",
  component: HamiriloFormDialog,
  parameters: {
    layout: "padded",
    docs: {
      description: {
...
- **\`loading\` は呼び出し側で管理する。**
  \`HamiriloConfirmDialog\` と違い、こちらは自動化していない
  （バリデーションエラーで開いたままにする等、制御が必要な場面が多いため）
- **エラー表示は自前で行う。** 全体エラーは \`children\` の先頭に置き、
  項目単位のエラーは \`HamiriloFormField\` の \`error\` に渡す
...
} satisfies Meta<typeof HamiriloFormDialog>;
  ...
  render: (args) => {
    ...
    return (
      <div className="space-y-3">
        <HamiriloButton variant="primary" onClick={() => setOpen(true)}>
          新規申請
        </HamiriloButton>
...
export const Disabled: Story = {
  render: (args) => {
    ...
    return (
      <>
        <HamiriloButton variant="secondary" onClick={() => setOpen(true)}>
          閲覧のみで開く
        </HamiriloButton>
        <HamiriloFormDialog
          {...args}
        ...
        >
          <div className="space-y-4">
            <HamiriloFormField label="件名">
              <HamiriloInput name="title" defaultValue="研修参加費" />
            </HamiriloFormField>
            <HamiriloFormField label="優先度">
              <HamiriloSelect name="priority" items={PRIORITY_ITEMS} defaultValue="high" />
            </HamiriloFormField>
          </div>
        </HamiriloFormDialog>
      </>
...
export const SuccessVariant: Story = {
  render: (args) => {
    ...
    return (
      <>
        <HamiriloButton variant="success" onClick={() => setOpen(true)}>
          公開設定
        </HamiriloButton>
        <HamiriloFormDialog
          {...args}
        ...
        >
          <div className="space-y-4">
            <HamiriloFormField label="公開範囲" required>
              <HamiriloSelect
                name="scope"
                items={[
                  { value: "all", label: "全社" },
    ...
    return (
      <>
        ...
        >
          <div className="space-y-4">
            <HamiriloFormField label="件名" required>
              <HamiriloInput name="title" required />
            </HamiriloFormField>
            <HamiriloFormField label="優先度" required>
              <HamiriloSelect name="priority" items={PRIORITY_ITEMS} placeholder="優先度を選択" />
            </HamiriloFormField>
            <HamiriloFormField label="金額" required helpText="税込">
              <HamiriloInput name="amount" type="number" min={0} />
            </HamiriloFormField>
