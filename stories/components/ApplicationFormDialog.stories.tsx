 ...
 */
const meta = {
  title: "Components/ApplicationFormDialog",
  component: ApplicationFormDialog,
  parameters: {
    layout: "padded",
    docs: {
      description: {
...
const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
  ...
  try {
    ...
    setOpen(false)
    ApplicationToast.success('作成しました')
  } catch (err) {
...
application のバリデーションエラーは JSON で返し、\`ApplicationFormField\` の \`error\` に流す
...
- **\`loading\` は呼び出し側で管理する。**
  \`ApplicationConfirmDialog\` と違い、こちらは自動化していない
  （バリデーションエラーで開いたままにする等、制御が必要な場面が多いため）
- **エラー表示は自前で行う。** 全体エラーは \`children\` の先頭に置き、
  項目単位のエラーは \`ApplicationFormField\` の \`error\` に渡す
...
} satisfies Meta<typeof ApplicationFormDialog>;
  ...
  render: (args) => {
    ...
    return (
      <div className="space-y-3">
        <ApplicationButton variant="primary" onClick={() => setOpen(true)}>
          新規申請
        </ApplicationButton>
 ...
 * 全体エラーは先頭のアラート、項目エラーは `ApplicationFormField` の `error` に流す。
...
export const Disabled: Story = {
  render: (args) => {
    ...
    return (
      <>
        <ApplicationButton variant="secondary" onClick={() => setOpen(true)}>
          閲覧のみで開く
        </ApplicationButton>
        <ApplicationFormDialog
          {...args}
        ...
        >
          <div className="space-y-4">
            <ApplicationFormField label="件名">
              <ApplicationInput name="title" defaultValue="研修参加費" />
            </ApplicationFormField>
            <ApplicationFormField label="優先度">
              <ApplicationSelect name="priority" items={PRIORITY_ITEMS} defaultValue="high" />
            </ApplicationFormField>
          </div>
        </ApplicationFormDialog>
      </>
...
export const SuccessVariant: Story = {
  render: (args) => {
    ...
    return (
      <>
        <ApplicationButton variant="success" onClick={() => setOpen(true)}>
          公開設定
        </ApplicationButton>
        <ApplicationFormDialog
          {...args}
        ...
        >
          <div className="space-y-4">
            <ApplicationFormField label="公開範囲" required>
              <ApplicationSelect
                name="scope"
                items={[
                  { value: "all", label: "全社" },
    ...
    return (
      <>
        ...
        >
          <div className="space-y-4">
            <ApplicationFormField label="件名" required>
              <ApplicationInput name="title" required />
            </ApplicationFormField>
            <ApplicationFormField label="優先度" required>
              <ApplicationSelect name="priority" items={PRIORITY_ITEMS} placeholder="優先度を選択" />
            </ApplicationFormField>
            <ApplicationFormField label="金額" required helpText="税込">
              <ApplicationInput name="amount" type="number" min={0} />
            </ApplicationFormField>
