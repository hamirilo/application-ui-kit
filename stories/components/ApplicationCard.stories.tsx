...
import { ApplicationButton, ApplicationCard, ApplicationDropdown } from "../../components/application";
...
- セマンティックトークン（\`bg-card\` / \`text-card-foreground\` / \`border-border\`）を
  使っているため、ApplicationCard 自体はダークモードで崩れない
        `,
      },
    },
  },
  ...
  args: {
    ...
    footer: (
      <>
        ...
        <ApplicationButton variant="primary">申請する</ApplicationButton>
      </>
    ),
  },
};
...
export const Grid: Story = {
  render: () => (
    <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
      <ApplicationCard title="未対応">
        <p className="text-2xl font-bold text-foreground">12</p>
        <p className="text-xs text-muted-foreground">件</p>
      </ApplicationCard>
      <ApplicationCard title="対応中">
        <p className="text-2xl font-bold text-foreground">5</p>
        <p className="text-xs text-muted-foreground">件</p>
      </ApplicationCard>
      <ApplicationCard title="今月の完了">
