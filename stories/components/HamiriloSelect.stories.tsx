...
import { HamiriloFormField, HamiriloSelect, type HamiriloSelectItem } from "../../components/hamirilo";

const PRIORITIES: HamiriloSelectItem[] = [
  { value: "high", label: "高" },
  { value: "mid", label: "中" },
  { value: "low", label: "低" },
];

const STATUSES: HamiriloSelectItem[] = [
  { value: "new", label: "未対応" },
...
const DEPARTMENTS: HamiriloSelectItem[] = [
...
/**
 * HamiriloSelect は選択肢から 1 つ選ぶためのコンポーネント。
 *
 * <important>
 ...
 */
const meta = {
  title: "Components/HamiriloSelect",
  component: HamiriloSelect,
  parameters: {
    layout: "padded",
    docs: {
      description: {
...
- **ラベルが必須。** トリガーは \`role="combobox"\` になる。ARIA はこのロールに
  ...
  \`HamiriloFormField\` で囲めばラベルが名前になる。単体で置くときは \`aria-label\` か
  \`aria-labelledby\` を渡す
...
- \`error\` は枠線の色だけを変える。メッセージは \`HamiriloFormField\` で表示する
- キーボード操作: \`Enter\`/\`Space\` で開く、矢印で移動、文字入力で先頭一致ジャンプ、\`Esc\` で閉じる
  ...
  args: {
    ...
    // 実画面では HamiriloFormField のラベルが名前になるため aria-label は不要。
    "aria-label": "優先度",
  },
  decorators: [
    (Story) => (
...
} satisfies Meta<typeof HamiriloSelect>;
 ...
 *
 * `error` を渡すと内側の `HamiriloSelect` にも自動で伝わる。
 */
export const WithFormField: Story = {
  render: () => (
    <div className="space-y-4">
      <HamiriloFormField label="優先度" required helpText="後から変更できます">
        <HamiriloSelect items={PRIORITIES} placeholder="優先度を選択" />
      </HamiriloFormField>

      <HamiriloFormField label="担当部署" required error="担当部署を選択してください">
        <HamiriloSelect items={DEPARTMENTS} placeholder="部署を選択" />
      </HamiriloFormField>
    </div>
  ),
};
...
export const AllStates: Story = {
  render: () => (
    <div className="space-y-3">
      <HamiriloSelect items={PRIORITIES} placeholder="未選択" aria-label="優先度（未選択）" />
      <HamiriloSelect items={PRIORITIES} defaultValue="high" aria-label="優先度（選択済み）" />
      <HamiriloSelect items={PRIORITIES} error placeholder="エラー" aria-label="優先度（エラー）" />
      <HamiriloSelect
        items={PRIORITIES}
        disabled
        defaultValue="low"
