...
const CATEGORIES: ApplicationTreeSelectItem[] = [
  {
    value: "expense",
    label: "経費",
    children: [
...
/**
 * ApplicationTreeSelect は階層構造から 1 つ選ぶためのコンポーネント。
 *
 * <important>
...
| 場面 | 代わりに使うもの |
|---|---|
| 選択肢がフラットで十数個まで | \`ApplicationSelect\` |
| 件数が多く「名前で検索して 1 つ選ぶ」 | \`ApplicationCombobox\` |
| 複数選択したい | \`ApplicationCombobox\` の \`multiple\` |
...
所有プロジェクト側は、API から取ったツリーを \`ApplicationTreeSelectItem[]\` に変換して渡す。
...
type ApplicationTreeSelectItem = {
  ...
  children?: ApplicationTreeSelectItem[]
}
\`\`\`
...
- **ラベルが必須。** トリガーは \`role="combobox"\` になるため、\`placeholder\` や選択中の値が
  あってもアクセシブルな名前は付かない。\`ApplicationFormField\` で囲むか
...
} satisfies Meta<typeof ApplicationTreeSelect>;
 ...
 *
 * 階層をたどらせる必然性が無いなら `ApplicationSelect` / `ApplicationCombobox` を使う。
 */
      ...
      >
        <Stack>
          <Labeled label="未選択（placeholder）">
            <ApplicationTreeSelect items={UNITS} placeholder="組織を選択" aria-label="未選択" />
          </Labeled>
          <Labeled label="選択済み（経路表示）">
            <ApplicationTreeSelect items={UNITS} value="core" aria-label="選択済み" />
          </Labeled>
          <Labeled label="エラー">
            <ApplicationTreeSelect items={UNITS} error placeholder="組織を選択" aria-label="エラー" />
          </Labeled>
          <Labeled label="操作不可">
            <ApplicationTreeSelect items={UNITS} value="sales" disabled aria-label="操作不可" />
          </Labeled>
        </Stack>
      </Section>
      ...
      >
        <Stack>
          <Labeled label="showPath（既定）">
            <ApplicationTreeSelect items={UNITS} value="core" aria-label="経路あり" />
          </Labeled>
          <Labeled label="末端のみ">
            <ApplicationTreeSelect items={UNITS} value="core" showPath={false} aria-label="末端のみ" />
          </Labeled>
        </Stack>
      </Section>
      ...
      >
        <Stack>
          <Labeled label="中間ノードも選べる（既定）">
            <ApplicationTreeSelect items={CATEGORIES} placeholder="勘定科目を選択" aria-label="科目" />
          </Labeled>
...
/**
 * 実際のフォームでの使い方。ラベルとエラーメッセージは `ApplicationFormField` が持つ。
 */
