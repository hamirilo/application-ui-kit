...
} satisfies Meta<typeof ApplicationCopyButton>;
      ...
      <Section title="States" note="押すと 2 秒間「コピーしました」に変わり、元へ戻る。">
        <Stack>
          <Labeled label="基本（secondary）">
            <ApplicationCopyButton value="コピーされる値" />
          </Labeled>
          <Labeled label="ラベル変更">
            <ApplicationCopyButton value="git clone ..." label="コマンドをコピー" />
          </Labeled>
          <Labeled label="primary">
            <ApplicationCopyButton value="コピーされる値" variant="primary" />
          </Labeled>
        </Stack>
      </Section>
    </Showcase>
...
export const WithVisibleValue: Story = {
  ...
  render: () => {
    ...
    const handleResult = (result: ApplicationCopyResult) => {
      setStatus(
