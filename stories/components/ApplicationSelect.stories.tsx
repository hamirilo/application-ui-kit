...
const STATUSES: ApplicationSelectItem[] = [
  { value: "new", label: "未対応" },
...
const DEPARTMENTS: ApplicationSelectItem[] = [
...
/**
 * ApplicationSelect は選択肢から 1 つ選ぶためのコンポーネント。
 *
 * <important>
 * 選択肢が十数個を超える、または検索が必要な場合は使わない。
 * React は ApplicationCombobox、application は application-specific select を使う。
 * </important>
 */
const meta = {
...
| 選択肢が多い / 検索が必要（社員・部署・取引先） | application-specific select（application） / \`ApplicationCombobox\`（React） |
...
- **ラベルが必須。** トリガーは \`role="combobox"\` になる。ARIA はこのロールに
  ...
  \`ApplicationFormField\` で囲めばラベルが名前になる。単体で置くときは \`aria-label\` か
  \`aria-labelledby\` を渡す
...
- キーボード操作: \`Enter\`/\`Space\` で開く、矢印で移動、文字入力で先頭一致ジャンプ、\`Esc\` で閉じる
  ...
  args: {
    ...
    // 実画面では ApplicationFormField のラベルが名前になるため aria-label は不要。
    "aria-label": "優先度",
  },
  decorators: [
    (Story) => (
...
} satisfies Meta<typeof ApplicationSelect>;
 ...
 *
 * `error` を渡すと内側の `ApplicationSelect` にも自動で伝わる。
 */
