...
import {
  ...
  HamiriloDropdown,
  type HamiriloDropdownItem,
  HamiriloToast,
} from "../../components/hamirilo";

/**
 * HamiriloDropdown は行アクションのメニュー。
 *
 * <important>
 ...
 */
const meta = {
  title: "Components/HamiriloDropdown",
  component: HamiriloDropdown,
  parameters: {
    layout: "padded",
    docs: {
      description: {
...
| 場面 | 代わりに使うもの |
|---|---|
| 選択肢から**値を選ぶ** | \`HamiriloSelect\`。ドロップダウンメニューは「操作」、セレクトは「入力」 |
| 操作が 1〜2 個 | ボタンを直接置く。1 個の操作をメニューに隠すと 2 クリックになる |
| 主要な操作（新規作成など） | 画面上部に \`HamiriloButton variant="primary"\` で常時表示する |
| ページ間の移動 | ナビゲーション。メニューに \`<a>\` を詰めるとリンクだと分からない |
| server-rendered markup（.html） | application JavaScript の \`x-show\` によるメニュー partial。React application を新設しない |
...
| Prop | 説明 |
|---|---|
| \`trigger\` | メニューを開く要素。**単一の React 要素**（通常は \`HamiriloButton\`） |
| \`items\` | 項目の配列 |
...
- **アイコンだけのトリガーには \`aria-label\` を付ける。**
  \`<HamiriloButton variant="ghost" size="icon" aria-label="操作メニュー">\` のように書く。
  付け忘れると読み上げが「ボタン」だけになる
- **\`danger: true\` は最後に置き、\`separatorBefore: true\` で区切る。**
  「編集」の隣に「削除」があると誤操作につながる
- **\`danger\` の項目から直接実行しない。** \`onSelect\` で
  \`HamiriloConfirmDialog\` を開く（Story の \`WithConfirmation\` を参照）
...
- 権限がない操作は \`disabled: true\` にするか、\`items\` から除外する。
  ...
  args: {
    trigger: (
      <HamiriloButton variant="ghost" size="icon" aria-label="操作メニュー">
...
} satisfies Meta<typeof HamiriloDropdown>;
...
export const TextTrigger: Story = {
  args: {
    trigger: (
      <HamiriloButton variant="secondary" rightIcon={<ChevronDown className="w-4 h-4" />}>
        操作
      </HamiriloButton>
    ),
    items: [
...
export const Alignment: Story = {
  render: () => {
    const items: HamiriloDropdownItem[] = [
      { key: "edit", label: "編集" },
      { key: "duplicate", label: "複製" },
    ];
        ...
        {ALIGNS.map((align) => (
          <div key={align} className="flex flex-col items-center gap-2">
            ...
            <HamiriloDropdown
              align={align}
              trigger={
                <HamiriloButton variant="secondary" size="sm">
                  開く
                </HamiriloButton>
              }
              items={items}
 ...
 * `onSelect` で `HamiriloConfirmDialog` を開き、そこで初めて実行する。
 * これが実アプリでの標準的な組み合わせ。
 */
          ...
          items={[
            {
              ...
              onSelect: () => HamiriloToast.info("編集画面を開きます"),
            },
            {
              key: "duplicate",
              label: "複製",
              icon: <Copy className="w-4 h-4" />,
              onSelect: () => HamiriloToast.success("複製しました"),
            },
            {
              key: "delete",
              label: "削除",
 ...
 * 一覧の行アクションとして使う例。
 *
 * 実際の配置はこの形になる。`HamiriloTable` の `WithActions` も参照。
 */
