 ...
 */
const meta = {
  title: "コンポーネント/ApplicationDatePicker",
...
} satisfies Meta<typeof ApplicationDatePicker>;
...
/**
 * `ApplicationFormField` と組み合わせる例。
 *
 * トリガーは `input-field` クラスを使っているため、
 * `ApplicationInput` と**同じ高さ・同じ枠**で並ぶ。
 */
export const InForm: Story = {
  render: (args) => {
    ...
    return (
      ...
      >
        <ApplicationFormField label="集計期間" required error={error}>
          <ApplicationDatePicker
            {...args}
            mode="range"
            value={range}
    ...
    return (
      <div className="space-y-3">
        <div className="flex flex-wrap gap-2">
          ...
          <ApplicationButton variant="ghost" size="sm" onClick={() => setRange(undefined)}>
            クリア
          </ApplicationButton>
        </div>
