...
/**
 * HamiriloDialog は汎用のモーダルダイアログ。
 *
 * <important>
 * 用途が決まっている場合は専用コンポーネントを使う。
 * 確認 → `HamiriloConfirmDialog` / フォーム → `HamiriloFormDialog`。
 * HamiriloDialog を直接使うのは、それらに当てはまらない場合だけ。
 * </important>
 */
const meta = {
  title: "Components/HamiriloDialog",
  component: HamiriloDialog,
  parameters: {
    layout: "padded",
    docs: {
      description: {
...
| 場面 | 代わりに使うもの |
|---|---|
| はい / いいえ の確認 | \`HamiriloConfirmDialog\`（アイコン・エラー表示・非同期処理が組み込み済み） |
...
} satisfies Meta<typeof HamiriloDialog>;
  ...
  render: (args) => {
    ...
    return (
      <>
        ...
        <HamiriloDialog {...args} open={open} onOpenChange={setOpen}>
 ...
 *
 * ただし削除確認は `HamiriloConfirmDialog type="danger"` を使うほうがよい
 * （アイコンとエラー表示が付く）。
 */
export const DangerVariant: Story = {
  render: (args) => {
    ...
    return (
      <>
        <HamiriloButton variant="danger" onClick={() => setOpen(true)}>
          削除
        </HamiriloButton>
        <HamiriloDialog
          {...args}
        ...
        <HamiriloDialog
          ...
          footer={
            <div className="flex w-full items-center justify-between gap-2">
              <HamiriloButton variant="ghost" onClick={() => setOpen(false)}>
                下書き保存
              </HamiriloButton>
              <div className="flex gap-2">
                <HamiriloButton variant="secondary" onClick={() => setOpen(false)}>
                  キャンセル
                </HamiriloButton>
                <HamiriloButton variant="primary" onClick={() => setOpen(false)}>
                  申請する
                </HamiriloButton>
              </div>
            </div>
    ...
    return (
      <>
        <div className="flex flex-wrap gap-2">
          {WIDTHS.map((w) => (
            <HamiriloButton key={w} variant="secondary" size="sm" onClick={() => setWidth(w)}>
              maxWidth=&quot;{w}&quot;
            </HamiriloButton>
          ))}
        </div>
