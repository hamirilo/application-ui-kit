...
import { ApplicationFormField, ApplicationTextarea } from "../../components/application";
 ...
 */
const meta = {
  title: "Components/ApplicationTextarea",
...
複数行入力の見た目・フォーカスリング・エラー表現を \`ApplicationInput\` と揃えて提供する。
...
- **\`error\` は色だけを変える。** 必ずエラーメッセージも表示する。\`ApplicationFormField\` を使えば両方自動で付く
...
} satisfies Meta<typeof ApplicationTextarea>;
...
export const WithFormField: Story = {
  render: () => (
    <div className="space-y-4">
      <ApplicationFormField label="申請理由" required helpText="200文字以内で入力してください">
        <ApplicationTextarea placeholder="例: 老朽化した椅子の買い替えのため" />
      </ApplicationFormField>

      <ApplicationFormField label="差戻し理由" required error="差戻し理由を入力してください">
        <ApplicationTextarea />
      </ApplicationFormField>
    </div>
  ),
