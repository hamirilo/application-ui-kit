...
/**
 * ApplicationFieldSet は「グループで選ぶ入力」にラベル・エラー・ヘルプを付ける部品。
 *
 * <important>
 * `<label for>` は labelable 要素にしか効かない。ラジオグループやボタングループへ
 * `ApplicationFormField` を使うと、**ラベルはあるのにアクセシブル名が付かない**。
 * </important>
 */
const meta = {
...
## ApplicationFormField との使い分け

| 子 | 使うもの | 名前の結び方 |
|---|---|---|
| 単一のコントロール（Input / Select / Combobox / TreeSelect / DatePicker / Textarea） | \`ApplicationFormField\` | \`<label for>\` → コントロールの id |
| グループ（RadioGroup / RadioTable / ButtonGroup） | **\`ApplicationFieldSet\`** | \`<legend>\` → \`aria-labelledby\` |
| チェックボックス 1 個 | どちらも使わない | \`ApplicationCheckbox\` が自分で \`label\` を持つ |
...
グループへ \`ApplicationFormField\` を使うと読み上げ名が付かない。
...
- グループ本体（\`ApplicationRadioGroup\` 等）へ \`aria-label\` を重ねて渡さない。名前が二重になる
- 必須は \`*\` だけでなく \`（必須）\` の文字でも伝える（この部品が自動で入れる）
  ...
  args: {
    label: "優先度",
    children: <ApplicationRadioGroup items={PRIORITIES} name="priority" />,
  },
} satisfies Meta<typeof ApplicationFieldSet>;
...
export const Overview: Story = {
  ...
  render: () => (
    <Showcase>
      ...
      >
        <Stack className="max-w-md">
          <ApplicationFieldSet label="優先度" required error="優先度を選択してください">
            <ApplicationRadioGroup items={PRIORITIES} name="priority-error" />
          </ApplicationFieldSet>

          <ApplicationFieldSet label="表示期間" required error="表示期間を選択してください">
            <ApplicationButtonGroup items={PERIODS} name="period-error" />
          </ApplicationFieldSet>
        </Stack>
      </Section>

      <Section
        title="ApplicationFormField との違い"
        note="上は名前が付く。下はラベルが見えていても、支援技術からは名前の無いグループになる。"
      >
        <Stack className="max-w-md">
          <Labeled label="ApplicationFieldSet（正）">
            <ApplicationFieldSet label="優先度">
              <ApplicationRadioGroup items={PRIORITIES} name="ok" />
            </ApplicationFieldSet>
          </Labeled>
          <Labeled label="ApplicationFormField（グループには使わない）">
            <ApplicationFormField label="優先度">
              <ApplicationRadioGroup items={PRIORITIES} name="ng" />
            </ApplicationFormField>
          </Labeled>
        </Stack>
      </Section>
...
export const WithButtonGroup: Story = {
  args: {
    label: "表示期間",
    required: true,
    children: <ApplicationButtonGroup items={PERIODS} name="period" defaultValue="week" />,
  },
};
