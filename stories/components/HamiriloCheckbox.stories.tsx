...
import { HamiriloButton, HamiriloCheckbox } from "../../components/hamirilo";

/**
 * HamiriloCheckbox はオン / オフを切り替えるためのコンポーネント。
 *
 * <important>
 ...
 */
const meta = {
  title: "Components/HamiriloCheckbox",
  component: HamiriloCheckbox,
  parameters: {
    layout: "padded",
    docs: {
      description: {
...
} satisfies Meta<typeof HamiriloCheckbox>;
    ...
    return (
      <div className="max-w-md space-y-3">
        ...
        <div className="flex items-center justify-between gap-3 border-t border-border pt-3">
          ...
          <HamiriloButton variant="primary" size="sm" disabled={selected.length === 0}>
            保存
          </HamiriloButton>
        </div>
      </div>
...
export const AllStates: Story = {
  render: () => (
    <div className="space-y-3">
      <HamiriloCheckbox label="未チェック" />
      <HamiriloCheckbox label="チェック済み" defaultChecked />
      <HamiriloCheckbox label="一部選択" indeterminate />
      <HamiriloCheckbox label="無効" disabled />
      <HamiriloCheckbox label="無効かつチェック済み" disabled defaultChecked />
      <HamiriloCheckbox label="説明付き" description="補足の説明文がここに入ります" />
    </div>
  ),
};
