 ...
 */
const meta = {
  title: "Components/ApplicationDropdown",
  component: ApplicationDropdown,
  parameters: {
    layout: "padded",
    docs: {
      description: {
...
| 場面 | 代わりに使うもの |
|---|---|
| 選択肢から**値を選ぶ** | \`ApplicationSelect\`。ドロップダウンメニューは「操作」、セレクトは「入力」 |
...
| Prop | 説明 |
|---|---|
| \`trigger\` | メニューを開く要素。**単一の React 要素**（通常は \`ApplicationButton\`） |
| \`items\` | 項目の配列 |
...
- **アイコンだけのトリガーには \`aria-label\` を付ける。**
  \`<ApplicationButton variant="ghost" size="icon" aria-label="操作メニュー">\` のように書く。
  付け忘れると読み上げが「ボタン」だけになる
- **\`danger: true\` は最後に置き、\`separatorBefore: true\` で区切る。**
  「編集」の隣に「削除」があると誤操作につながる
- **\`danger\` の項目から直接実行しない。** \`onSelect\` で
  \`ApplicationConfirmDialog\` を開く（Story の \`WithConfirmation\` を参照）
...
} satisfies Meta<typeof ApplicationDropdown>;
...
export const Alignment: Story = {
  render: () => {
    const items: ApplicationDropdownItem[] = [
      { key: "edit", label: "編集" },
      { key: "duplicate", label: "複製" },
    ];
 ...
 * `onSelect` で `ApplicationConfirmDialog` を開き、そこで初めて実行する。
 * これが実アプリでの標準的な組み合わせ。
 */
 ...
 * 一覧の行アクションとして使う例。
 *
 * 実際の配置はこの形になる。`ApplicationTable` の `WithActions` も参照。
 */
