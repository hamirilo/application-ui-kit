...
/**
 * ApplicationButtonGroup は排他的な選択（どれか1つ）を、隣接したボタンの並び
 * （segmented control）として表現するコンポーネント。
 *
 * <important>
 * 内部的には `ApplicationRadioGroup` と同じくラジオボタン。「複数選べる」ようには見えるが
 * 実際には 1 つしか選べないため、複数選択が必要な場面では使わない。
 * </important>
 */
const meta = {
  title: "Actions/ApplicationButtonGroup",
...
見た目のボタングループだが、選択の意味は \`ApplicationRadioGroup\` と同じ（どれか1つ）。
...
| 補足説明を選択肢ごとに添えたい | \`ApplicationRadioGroup\`（縦に並ぶ分、説明文を置ける） |
| ページ内のコンテンツ切り替え（タブ） | \`ApplicationTabs\` |
...
} satisfies Meta<typeof ApplicationButtonGroup>;
  ...
  render: (args) => {
    ...
    return (
      <div className="space-y-3">
        <ApplicationButtonGroup {...args} value={value} onValueChange={setValue} />
