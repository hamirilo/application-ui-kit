...
const TAGS: ApplicationComboboxItem[] = [
...
/**
 * ApplicationCombobox は、入力して絞り込みながら選ぶためのコンポーネント。
 *
 * <important>
 ...
 */
const meta = {
  title: "Components/ApplicationCombobox",
  component: ApplicationCombobox,
  parameters: {
    layout: "padded",
    docs: {
      description: {
...
見た目は \`ApplicationSelect\` と同じ枠（\`select.input-field\`）に揃えてある。
...
\`__create__:○○\`（\`APPLICATION_COMBOBOX_CREATE_PREFIX\`）が入る。
...
- **ラベルが必須。** 入力欄は \`role="combobox"\` になる。ARIA はこのロールに
  ...
  アクセシブルな名前は付かない。\`ApplicationFormField\` で囲むか、
...
- \`error\` は枠線の色だけを変える。メッセージは \`ApplicationFormField\` で表示する
...
} satisfies Meta<typeof ApplicationCombobox>;
...
export const WithFormField: Story = {
  render: () => (
    <div className="space-y-4">
      <ApplicationFormField label="店舗" required helpText="店名の一部で絞り込めます">
        <ApplicationCombobox items={SHOPS} placeholder="店舗を検索" />
      </ApplicationFormField>
      <ApplicationFormField label="タグ" error="1 つ以上選んでください">
        <ApplicationCombobox items={TAGS} multiple creatable placeholder="タグを検索・追加" error />
      </ApplicationFormField>
    </div>
  ),
};
...
export const WithCreatedValue: Story = {
  args: {
    ...
    multiple: true,
    creatable: true,
    defaultValue: ["t1", `${APPLICATION_COMBOBOX_CREATE_PREFIX}新しいタグ`],
    "aria-label": "タグ",
  },
};
