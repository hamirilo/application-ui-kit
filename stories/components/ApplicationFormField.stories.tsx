 ...
 */
const meta = {
  title: "コンポーネント/ApplicationFormField",
...
- application フォームと連携する場合は \`htmlFor={field.id_for_label}\` を渡して id を揃える
  ...
  args: {
    label: "件名",
    children: <ApplicationInput placeholder="例: 備品購入の申請" />,
  },
  decorators: [
    (Story) => (
...
} satisfies Meta<typeof ApplicationFormField>;
 ...
 * エラー状態。
 *
 * `error` を渡すだけで、子の `ApplicationInput` が赤枠になり `aria-invalid` が付き、
...
export const WithError: Story = {
  args: {
    ...
    children: <ApplicationInput defaultValue="foo@" />,
  },
};
...
export const WithErrorAndHelpText: Story = {
  args: {
    ...
    children: <ApplicationInput type="password" defaultValue="abc" />,
  },
};

/** `ApplicationSelect` と組み合わせる例。 */
 ...
 * `id` / `aria-describedby` は `ApplicationFormField` が注入するが、
  ...
  render: () => (
    <form className="space-y-4">
      <ApplicationFormField label="件名" required helpText="50 文字以内で入力してください">
        <ApplicationInput placeholder="例: 備品購入の申請" required />
      </ApplicationFormField>
      ...
      <ApplicationFormField label="金額" required error="金額を入力してください">
        <ApplicationInput type="number" placeholder="0" min={0} />
      </ApplicationFormField>

      <ApplicationFormField label="備考" helpText="任意">
        <ApplicationInput placeholder="補足があれば記入してください" />
      </ApplicationFormField>
