...
const COLUMNS: ApplicationTableColumn<Plan>[] = [
...
/**
 * ApplicationRadioTable は「表から1行を選ばせる」コンポーネント。
 *
 * <important>
 * 一覧を見せることが目的なら `ApplicationTable` を使う。こちらは
 * **選んだ1件をフォームの値として送る** ためのもの。
 * </important>
 */
/** 総称型のため、Story の args は Plan で固定した props 型で受ける。 */
type PlanTableProps = ApplicationRadioTableProps<Plan>;

const meta = {
  title: "Components/ApplicationRadioTable",
  component: ApplicationRadioTable,
  parameters: {
    layout: "padded",
    docs: {
      description: {
...
| 場面 | 代わりに使うもの |
|---|---|
| 一覧を見せるだけ（選択しない） | \`ApplicationTable\` |
| 候補ごとに1〜2行の説明で足りる | \`ApplicationRadioGroup variant="cards"\` |
| ラベルだけで決められる | \`ApplicationRadioGroup\` / \`ApplicationSelect\` |
| 複数選びたい | チェックボックス列を持つ一覧（この部品は排他選択専用） |

## Props

- 列は \`ApplicationTable\` と同じ \`columns\`（ラジオ列は自動で先頭に足される）
...
- 行数が多い一覧には向かない。検索が必要なら \`ApplicationCombobox\` を検討する
        `,
      },
    },
  },
  ...
  render: (args) => {
    ...
    return (
      <div className="space-y-3">
        <ApplicationRadioTable<Plan> {...args} value={value} onValueChange={setValue} />
