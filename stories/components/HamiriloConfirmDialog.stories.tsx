...
import { HamiriloButton, HamiriloConfirmDialog } from "../../components/hamirilo";

/**
 * HamiriloConfirmDialog は「実行してよいか」を尋ねる確認ダイアログ。
 *
 * <important>
 ...
 */
const meta = {
  title: "Components/HamiriloConfirmDialog",
  component: HamiriloConfirmDialog,
  parameters: {
    layout: "padded",
    docs: {
      description: {
...
| 入力を伴う確認（理由を書かせる等） | \`HamiriloFormDialog\` |
| 元に戻せる操作（並び替え、下書き保存） | 確認しない。実行して \`HamiriloToast\` で伝える |
| 操作の結果を知らせるだけ | \`HamiriloToast\`。確認ダイアログは「問い」であり「報告」ではない |
| server-rendered markup（.html） | server-rendering の確認 partial か、各プロジェクトのモーダル。React application を新設しない |
...
} satisfies Meta<typeof HamiriloConfirmDialog>;
  ...
  render: (args) => {
    ...
    return (
      <div className="space-y-3">
        <HamiriloButton variant="danger" onClick={() => setOpen(true)} disabled={deleted}>
