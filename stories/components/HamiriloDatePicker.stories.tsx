...
import { HamiriloButton, HamiriloDatePicker, HamiriloFormField } from "../../components/hamirilo";

/**
 * HamiriloDatePicker は日付を選ぶコンポーネント。
 *
 * <important>
 ...
 */
const meta = {
  title: "Components/HamiriloDatePicker",
  component: HamiriloDatePicker,
  parameters: {
    layout: "padded",
    docs: {
      description: {
...
- トリガーは \`input-field\` クラスを使っているため、\`HamiriloInput\` と同じ高さで並ぶ
        `,
      },
    },
  },
...
} satisfies Meta<typeof HamiriloDatePicker>;
...
/**
 * `HamiriloFormField` と組み合わせる例。
 *
 * トリガーは `input-field` クラスを使っているため、
 * `HamiriloInput` と**同じ高さ・同じ枠**で並ぶ。
 */
export const InForm: Story = {
  render: (args) => {
    ...
    return (
      ...
      >
        <HamiriloFormField label="集計期間" required error={error}>
          <HamiriloDatePicker
            {...args}
            mode="range"
            value={range}
    ...
    return (
      <div className="space-y-3">
        <div className="flex flex-wrap gap-2">
          ...
          <HamiriloButton variant="ghost" size="sm" onClick={() => setRange(undefined)}>
            クリア
          </HamiriloButton>
        </div>
