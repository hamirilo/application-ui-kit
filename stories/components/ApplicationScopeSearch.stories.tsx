 ...
 */
const ITEMS: ApplicationScopeSearchItem[] = [
  {
    value: "10024",
    kind: "社員",
    label: "青木 里佐",
...
/**
 * ApplicationScopeSearch は「何を探しているか決めないまま打ち始める」検索窓。
 *
 * <important>
 * フォームの値を決める部品ではない。選んだものをフォームに入れるなら
 * `ApplicationCombobox` / `ApplicationTreeSelect` を使う。
 * </important>
 */
const meta = {
  title: "コンポーネント/ApplicationScopeSearch",
...
| 場面 | 代わりに使うもの |
|---|---|
| 選んだ値をフォームに入れる | \`ApplicationCombobox\`（1 種類の候補から選ぶ） |
| 階層から選ぶ | \`ApplicationTreeSelect\` |
| 一覧の絞り込み（同じ画面に留まる） | \`ApplicationSearchInput\` + \`Patterns/Search\` |
| 種別が 1 つだけ | \`ApplicationCombobox\`。種別チップが無意味になる |
...
export const Controlled: Story = {
  ...
  render: (args) => {
    ...
    const [chosen, setChosen] = React.useState<ApplicationScopeSearchItem | null>(null);
    ...
    return (
      <div className="max-w-lg space-y-3">
        ...
        <p className="text-sm text-muted-foreground">
          {chosen ? (
            <>
              <ApplicationBadge tone="neutral">{chosen.kind}</ApplicationBadge>{" "}
...
export const ServerSide: Story = {
  ...
  render: (args) => {
    const [query, setQuery] = React.useState("");
    const [results, setResults] = React.useState<ApplicationScopeSearchItem[]>([]);
