...
/**
 * HamiriloFormField はラベル・入力欄・エラー・ヘルプをまとめる部品。
 *
 * <important>
 ...
 */
const meta = {
  title: "Components/HamiriloFormField",
  component: HamiriloFormField,
  parameters: {
    layout: "padded",
    docs: {
      description: {
...
- application フォームと連携する場合は \`htmlFor={field.id_for_label}\` を渡して id を揃える
  ...
  args: {
    label: "件名",
    children: <HamiriloInput placeholder="例: 備品購入の申請" />,
  },
  decorators: [
    (Story) => (
...
} satisfies Meta<typeof HamiriloFormField>;
 ...
 * エラー状態。
 *
 * `error` を渡すだけで、子の `HamiriloInput` が赤枠になり `aria-invalid` が付き、
...
export const WithError: Story = {
  args: {
    ...
    children: <HamiriloInput defaultValue="foo@" />,
  },
};
...
export const WithErrorAndHelpText: Story = {
  args: {
    ...
    children: <HamiriloInput type="password" defaultValue="abc" />,
  },
};

/** `HamiriloSelect` と組み合わせる例。 */
 ...
 * `id` / `aria-describedby` は `HamiriloFormField` が注入するが、
  ...
  render: () => (
    <form className="space-y-4">
      <HamiriloFormField label="件名" required helpText="50 文字以内で入力してください">
        <HamiriloInput placeholder="例: 備品購入の申請" required />
      </HamiriloFormField>
      ...
      <HamiriloFormField label="金額" required error="金額を入力してください">
        <HamiriloInput type="number" placeholder="0" min={0} />
      </HamiriloFormField>

      <HamiriloFormField label="備考" helpText="任意">
        <HamiriloInput placeholder="補足があれば記入してください" />
      </HamiriloFormField>
