 ...
 *
 * <important>
 * `ApplicationTable` は**表を描くだけ**。検索・絞り込み・並び替え・ページングは業務ロジックなので
...
4. ApplicationTable                                 ← 表本体（空状態は必ず設定）
5. 件数表示 + ページング                   ← 表の「下」
\`\`\`

| 決めごと | ルール |
|---|---|
| 主アクション | 見出しの右。\`ApplicationButton variant="primary"\` + \`leftIcon\` |
| 検索欄 | 表の上。\`ApplicationInput leftIcon={<Search />}\`。**ラベルは置かず \`aria-label\` を付ける** |
| 絞り込みの解除 | 条件が付いているときだけ「クリア」を出す |
| 一括操作 | 表の**上**に置く（下だとスクロールで見えなくなる） |
| 行アクション | 右端の \`ApplicationDropdown\`。3 つ以上ならメニューに畳む |
| 削除 | \`ApplicationDropdown\` から直接実行せず \`ApplicationConfirmDialog\` を経由 |
...
| 表計算のような編集 | ApplicationTable の範囲外。専用 UI を検討する |
...
詳しい判断軸と、他の選択肢との比較は \`patterns/data-list.md\` を参照してください。
`,
      },
    },
  },
...
function baseColumns(): ApplicationTableColumn<Request>[] {
  return [
    {
      key: "code",
      header: "申請番号",
...
/** 行アクション列（メニュー + 削除確認）。 */
function actionColumn(onDelete: (row: Request) => void): ApplicationTableColumn<Request> {
  return {
    key: "actions",
 ...
 * - 削除は `ApplicationDropdown` から直接実行せず `ApplicationConfirmDialog` を経由
      ...
      <div className="space-y-4">
        ...
        <ApplicationConfirmDialog
          ...
          onConfirm={async () => {
            ...
            setTarget(null);
            ApplicationToast.success("削除しました");
          }}
        />
        ...
        <ApplicationConfirmDialog
          ...
          onConfirm={async () => {
            ...
            setSelected([]);
            // ✓ 1 件ごとに出さず、まとめて 1 回
            ApplicationToast.success(`${count} 件を削除しました`);
          }}
        />
      </div>
    );
 ...
 * 並び替えとページング。
 *
 * **どちらも `ApplicationTable` は持っていない。** 画面側で計算し、結果を `rows` に渡す。
...
export const WithSortingAndPaging: Story = {
  render: () => {
     ...
     * 並び替えボタン付きのヘッダー。
     *
     * ApplicationTable は `<th>` に `aria-sort` を付ける手段を持たないため、
