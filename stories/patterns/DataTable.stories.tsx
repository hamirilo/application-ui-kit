 ...
 *
 * <important>
 * `HamiriloTable` は**表を描くだけ**。検索・絞り込み・並び替え・ページングは業務ロジックなので
...
4. HamiriloTable                                 ← 表本体（空状態は必ず設定）
5. 件数表示 + ページング                   ← 表の「下」
\`\`\`

| 決めごと | ルール |
|---|---|
| 主アクション | 見出しの右。\`HamiriloButton variant="primary"\` + \`leftIcon\` |
| 検索欄 | 表の上。\`HamiriloInput leftIcon={<Search />}\`。**ラベルは置かず \`aria-label\` を付ける** |
| 絞り込みの解除 | 条件が付いているときだけ「クリア」を出す |
| 一括操作 | 表の**上**に置く（下だとスクロールで見えなくなる） |
| 行アクション | 右端の \`HamiriloDropdown\`。3 つ以上ならメニューに畳む |
| 削除 | \`HamiriloDropdown\` から直接実行せず \`HamiriloConfirmDialog\` を経由 |
...
- React application で一覧を持つ画面（絞り込みを JS で完結させたい場合）
...
function baseColumns(): HamiriloTableColumn<Request>[] {
  return [
    {
      key: "code",
      header: "申請番号",
...
/** 行アクション列（メニュー + 削除確認）。 */
function actionColumn(onDelete: (row: Request) => void): HamiriloTableColumn<Request> {
  return {
    key: "actions",
    ...
    cell: (row) => (
      <HamiriloDropdown
        label={row.code}
        trigger={
          <HamiriloButton variant="ghost" size="icon" aria-label={`${row.title} の操作メニュー`}>
  ...
  render: () => (
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-3">
        ...
        <HamiriloButton variant="primary" leftIcon={<Plus className="w-4 h-4" />}>
          新規申請
        </HamiriloButton>
      </div>

      <HamiriloTable<Request>
        columns={baseColumns()}
        rows={ALL_ROWS}
        rowKey={(r) => r.id}
        caption="申請の一覧"
 ...
 * - 削除は `HamiriloDropdown` から直接実行せず `HamiriloConfirmDialog` を経由
...
export const WithActionsAndSelection: Story = {
  render: () => {
    ...
    const columns: HamiriloTableColumn<Request>[] = [
      {
        key: "select",
        className: "w-10",
        header: (
      ...
      <div className="space-y-4">
        ...
        <HamiriloConfirmDialog
          ...
          onConfirm={async () => {
            ...
            setSelected([]);
            // ✓ 1 件ごとに出さず、まとめて 1 回
            HamiriloToast.success(`${count} 件を削除しました`);
          }}
        />
      </div>
    );
 ...
 * 並び替えとページング。
 *
 * **どちらも `HamiriloTable` は持っていない。** 画面側で計算し、結果を `rows` に渡す。
...
export const WithSortingAndPaging: Story = {
  render: () => {
     ...
     * 並び替えボタン付きのヘッダー。
     *
     * HamiriloTable は `<th>` に `aria-sort` を付ける手段を持たないため、
  ...
  render: () => (
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-2">
        ...
        <HamiriloButton variant="primary" size="sm" leftIcon={<Plus className="w-4 h-4" />}>
          新規
        </HamiriloButton>
      </div>
