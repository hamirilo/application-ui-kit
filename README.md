...
mainへマージするとCIが `storybook-static` をworkflow artifactとして上げ、配信側（UI Catalog）が
それを取りに来て配ります。他リポジトリのStorybookと横断で見るにはUI Catalogを使います。

Catalogの構成・配信URL・リポジトリの追加手順は `ui-catalog` repositoryのREADMEを正とします。
